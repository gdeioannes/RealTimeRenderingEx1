...
bool CheckPoint(Point2D a,Point2D b,Point2D c, Point2D p){
	...
	if((ORI (b,c,p) == ORI (c,a,p) &&
			ORI(a,b,p) == ORI (b,c,p)) ||
			CheckPointLine(a,b,c,p) ||
			CheckPointLine(b,c,a,p) ||
			CheckPointLine(c,a,b,p)){
		return true;
	}
	return false;
}
